...
def train(opt):
    ...
    model = HierAttNet(opt.word_hidden_size, opt.sent_hidden_size, opt.batch_size, 10,
                       opt.word2vec_path, max_sent_length, max_word_length)

    model.load_state_dict(torch.load(opt.pretrain_model),strict=False)
    model.sent_att_net.fc = nn.Linear(2 * opt.sent_hidden_size, training_set.num_classes)
